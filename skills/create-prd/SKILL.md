---
name: create-prd
description: 根据用户提供的业务上下文，生成结构化的 B端 PRD 文档（含初始内容）。当用户要求创建、撰写、生成 PRD、需求文档、产品方案、系统设计文档时自动触发，即使用户只说"帮我写个 PRD/方案/需求文档"。
---

# create-prd

根据用户提供的业务上下文和需求背景，生成结构化的、带有初始内容的 B端 PRD 文档。

支持显式调用（如 `/create-prd`）和自动触发（当用户明确要求创建、撰写 PRD、需求文档、产品方案或企业系统设计时）。

## 输入

- 用户提供的业务上下文和需求背景（自由文本、会议纪要、简要描述等均可）
- 或通过 `$ARGUMENTS` 传入的文件路径

如有参数传入，优先作为输入源：

$ARGUMENTS

## 生成流程

先识别实际范围，再生成相应内容并自检。阶段是组织工作的方法，不是逐步等待用户批准的关卡；模块和单点请求只执行适用分支。

### 阶段 0：理解上下文与产品定型

1. 完整阅读并理解用户提供的所有业务上下文。
2. 从用户描述中推断：
   - **商业属性**：商业化产品 or 企业自研系统
   - **功能类型**：业务型管理软件 / 工具型软件 / 交易型平台 / 基础服务型
   - **文档范围**：0-1 完整系统 / 系统级重构或大版本 / 模块级迭代 / 单点功能变更 / 快速产品说明
3. 必要时用一句话说明判断与依据，然后继续：

> 根据你的描述，这是一个【{商业属性} × {功能类型} × {文档范围}】需求{，简要理由}。我会据此调整 PRD 的完整度和章节侧重点。如有不对请纠正。

4. 已有明确背景或可合理推断时直接写作；只有缺失信息会实质改变目标、核心范围或业务规则，且无法从资料查明时才澄清。可独立完成的内容继续写，真实未知项标为 `[TODO]`。
5. 加载定型参考文件，确定适用章节，不要求用户确认分类标签：

[产品定型与章节适配](references/appendices/create-prd-appendix-typing.md)

6. 根据文档范围选择输出分支：
   - **完整 PRD 分支**：0-1 完整系统、系统级重构或大版本，使用第1-14章完整结构。
   - **模块迭代 PRD 分支**：模块级迭代，使用精简结构，重点覆盖变更背景、目标、范围、流程、页面交互、字段规则、状态规则、权限、异常、验收与待决。
   - **单点功能 PRD 分支**：单点功能变更或快速产品说明，使用轻量结构，重点覆盖问题、方案、影响范围、交互/字段/规则、验收、风险。

### 文档范围分支规则

#### 完整 PRD 分支

适用：0-1 完整系统、系统级重构、大版本规划、复杂跨模块项目。

继续执行阶段 1 至阶段 4 的完整 14 章流程。

#### 模块迭代 PRD 分支

适用：已有系统中的单模块新增、改造、流程优化，或涉及多个页面但不重建整体系统的需求。

输出结构：

```md
# {模块/需求名称} PRD

## 1、变更背景与目标
## 2、需求范围与不做范围
## 3、业务场景与用户路径
## 4、变更方案概述
## 5、功能需求详解
### 5.1 业务流程
### 5.2 页面交互
### 5.3 字段与状态规则
### 5.4 业务规则
## 6、权限与数据影响
## 7、异常处理与兼容方案
## 8、验收标准
## 9、待决事项
```

生成要求：
- 不强制输出商业分析、完整系统架构、完整 ER 图、完整运营计划。
- 如果影响核心实体、状态流转或跨模块数据，必须在第5章补充局部数据模型、状态机和状态字段可编辑性矩阵。
- 如果只影响页面展示或轻量操作，可用字段表、操作表和规则表替代完整架构图。

#### 单点功能 PRD 分支

适用：单个按钮、字段、校验、导入导出、展示优化、权限开关、文案或流程小改。

输出结构：

```md
# {功能变更名称} PRD

## 1、问题与目标
## 2、方案说明
## 3、影响范围
## 4、交互、字段与规则
## 5、异常与兼容
## 6、验收标准
## 7、风险与待决
```

生成要求：
- 不强制补齐 14 章。
- 不强制输出系统框架图、ER 图、运营计划、完整权限矩阵。
- 必须讲清楚改哪个入口、哪个页面、哪个字段、哪个规则、对哪些角色和状态生效。
- 有状态实体时，仍必须补充状态字段可编辑性或说明“不涉及状态差异”。

### 阶段 1：前置章节（1-9章）

按需要组织第1至第9章。默认完成完整文档后统一交付，长文写入单个文件；只有用户要求逐章审看时才分章交付，不把每章结束当作整个任务结束。

每章加载对应的生成指引：

1. [第1章 项目背景](references/chapters/create-prd-ch01-background.md)
2. [第2章 需求基本情况](references/chapters/create-prd-ch02-basic.md)
3. [第3章 商业分析](references/chapters/create-prd-ch03-commercial.md)
4. [第4章 项目收益目标](references/chapters/create-prd-ch04-goals.md)
5. [第5章 项目方案概述](references/chapters/create-prd-ch05-overview.md)
6. [第6章 项目范围](references/chapters/create-prd-ch06-scope.md)
7. [第7章 项目风险](references/chapters/create-prd-ch07-risks.md)
8. [第8-9章 术语与参考文献](references/chapters/create-prd-ch08-09-terms.md)

### 阶段 2：核心功能需求（第10章）

这是 PRD 中最大、最核心的章节。加载生成指引：

9. [第10章 功能需求](references/chapters/create-prd-ch10-functions.md)

按子章节分段生成：
- 10.1 产品框架概述（系统框架图、数据模型图、业务流程图、状态机图、功能清单）
- 10.2 产品需求详解（逐模块：流程图 → 页面交互 → 业务规则）
- 10.3 异常情况处理方案

### 阶段 3：后置章节（11-14章）

10. [第11章 数据埋点](references/chapters/create-prd-ch11-tracking.md)
11. [第12章 角色和权限](references/chapters/create-prd-ch12-permissions.md)
12. [第13章 运营计划](references/chapters/create-prd-ch13-operations.md)
13. [第14章 待决事项](references/chapters/create-prd-ch14-tbd.md)

### 阶段 4：自检与缺口分析

所有章节生成完毕后，执行轻量自检：

14. [自检与待完善清单](references/appendices/create-prd-appendix-selfcheck.md)

## 输出规范

### 文档格式

以单个 Markdown 文档输出 PRD，结构如下：

```md
# {产品/项目名称} PRD

| PRD 审核人 | {待填写} |
| --- | --- |
| 重要性 | {高/中/低} |
| 紧迫性 | {高/中/低} |
| 需求方 | {从上下文推断或标注待填写} |
| PRD 编写人 | {用户姓名或待填写} |
| PRD 提交日期 | {当前日期} |

## PRD 修改记录

| 变更时间 | 变更内容 | 变更提出部门与理由 | 修改人 | 审核人 | 版本号 |
| --- | --- | --- | --- | --- | --- |
| {当前日期} | 初始版本 | — | {编写人} | {待填写} | v1.0 |

---

## 1、项目背景
...（各章节内容）

## 14、待决事项
...

---

## 附：待完善清单
...
```

### 内容生成规则

1. **有信息则生成实质内容**：根据用户提供的上下文，尽可能生成具体、有实质内容的初稿。
2. **信息不足则标注 `[TODO]`**：对于用户未提供足够信息的部分，用 `[TODO: 具体需要补充什么]` 标注，而不是编造内容。
3. **区分产品类型**：商业化产品与企业自研系统的内容侧重点不同，严格按照产品定型结果调整。
4. **方法服务判断**：只有解释关键取舍时才简述依据，不为展示方法论添加固定提示块。
5. **结构化优先**：表格、列表、Mermaid 图表优先于大段叙述。
6. **图表使用 Mermaid**：架构图、流程图、状态机、ER 模型等全部使用 Mermaid 代码块生成，不使用 ASCII 伪图。
7. **按范围控制复杂度**：完整系统使用完整 14 章；模块迭代和单点功能按分支精简，不为小需求强行补齐商业分析、完整架构或运营计划。
8. **状态字段规则不可省略**：涉及订单、工单、审批、任务、发布、结算、导入任务等状态实体时，必须说明每个状态下字段和操作的可编辑性。

### 图表生成规则

第10章按业务实体、流程与状态选择必要图表；所需图表使用 Mermaid 语法。没有对应对象时不补造角色、实体或流程来凑图表种类：

| 图表类型 | Mermaid 语法 | 必须包含 |
| --- | --- | --- |
| 应用架构图 | `graph TB` + `subgraph` 分层 | 用户层、接入层、业务服务层、数据层、外部系统 |
| ER 数据模型 | `erDiagram` | 所有核心实体+关系+关键属性（PK/FK/状态） |
| 业务流程图 | `flowchart TD` | 主流程+关键分支+异常路径，关键节点着色 |
| 状态机图 | `stateDiagram-v2` | 正常+异常路径，附 note 说明约束 |

**注意事项：**
- Mermaid 图后面附对应的明细表格作为补充说明（如状态机图+状态转换表）
- 有状态实体必须在状态机图和状态转换表后补充“状态字段可编辑性矩阵”，说明不同状态下各字段、按钮和操作是否可见、可编辑、可触发
- 图表内节点文字用 `<br/>` 换行，保持简洁
- 具体模板和示例见第10章生成指引文件

### 交付规则

- 默认交付一份连贯的完整文档；只有用户要求分章审看或指定阶段时才在相应位置暂停。
- 每章必须有清晰的章节标题，与 PRD 模板结构一致。
- 结构化数据（字段、权限、规则等）优先使用表格。
- 方法论说明按需要使用，不作为每章必填内容。
- 不确定的内容用 `[TODO]` 标注，并说明需要补充什么信息。

## 工作风格

- 目标是完成用户需要的可用 PRD；资料足够时交付实质正文，不停在目录、脚手架或执行计划。未获证据的业务事实和用户最终决策不能由模型编造。
- 用户上下文充分时，尽量具体和有实质内容；信息不足时，坦诚标注缺口。
- 保持专业的 PRD 写作风格：精确、结构化、无歧义。
- 根据用户提供的上下文丰富度调整深度——一段话的上下文生成轻量 PRD，详细上下文生成丰富 PRD。
- 如果用户提供的上下文非常有限，生成结构框架并附带指引说明，主动询问哪些补充信息有助于充实关键章节。
