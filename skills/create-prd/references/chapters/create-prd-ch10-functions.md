...
- 状态机图后必须附“状态字段可编辑性矩阵”，说明每个状态下字段、按钮、入口和操作是否可见、可编辑、可触发

状态转换明细表模板：

| 当前状态 | 触发事件 | 目标状态 | 操作角色 | 前置条件 | 后置影响 | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| {状态A} | {提交/审核/驳回} | {状态B} | {角色} | {触发前必须满足的条件} | {状态、字段、通知、日志变化} | |

状态字段可编辑性矩阵模板：

| 核心实体 | 状态 | 字段/操作 | 字段类型 | 是否可见 | 是否可编辑/可触发 | 可操作角色 | 触发条件 | 校验规则 | 变更后影响 | 审计记录 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| {实体} | {待提交} | {字段A} | 文本/枚举/金额/日期/按钮 | 是 | 是 | {创建人/管理员} | {条件} | {必填/格式/范围/唯一性} | {影响状态/金额/通知/关联数据} | 记录字段变更前后值 |
| {实体} | {审核中} | {字段A} | 文本/枚举/金额/日期/按钮 | 是 | 否/只读 | {审批人} | {条件} | {规则} | {影响} | 记录/不记录 |

**矩阵生成规则：**
- 行维度优先按“状态 × 字段/操作”展开；状态多、字段多时，可按字段分组，但不能省略有差异的状态。
- 字段和操作都要覆盖：表单字段、列表行操作、详情页按钮、批量操作、导入导出、审批操作、撤回/作废/删除等。
- 对同一字段在不同角色下可编辑性不同的场景，必须拆成多行说明。
- 对“可编辑”必须写明校验规则；对“不可编辑/只读”必须说明原因或前置约束。
- 状态变化会联动字段、消息、日志、权限、库存、额度、金额等数据时，必须在“变更后影响”中说明。
- 如确认不涉及状态差异，写明：“本模块不涉及状态字段可编辑性差异，各字段规则在所有状态下保持一致。”
...
| 状态 | 字段/操作 | 是否可见 | 是否可编辑/可触发 | 可操作角色 | 触发条件 | 校验/限制 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| | | 是/否 | 是/否/只读 | | | | |
...
5. **状态字段可编辑性矩阵**：有状态实体必须说明不同状态下字段、按钮和操作入口的可见、可编辑、可触发规则。
6. **功能清单**：用表格列出所有页面和多端支持情况。
...
4. 涉及状态差异时，页面交互必须补充状态字段可编辑性表；没有状态差异时显式说明“不涉及状态字段可编辑性差异”。
5. 如果信息充足，生成具体字段和规则；如果不足，生成表格框架并标注 `[TODO]`。

### 模块级/单点迭代的简化规则

当阶段 0 判断为“模块级迭代”或“单点功能变更”时，第10章不强制输出完整 10.1 系统框架概述，但必须保留以下内容：

| 文档范围 | 第10章最低输出 |
| --- | --- |
| 模块级迭代 | 受影响模块说明、局部业务流程、页面交互、字段规则、状态字段可编辑性（如适用）、业务规则、异常处理 |
| 单点功能变更 | 入口/页面位置、变更前后差异、字段/按钮/校验规则、状态和权限差异、异常兼容、验收标准 |
| 快速产品说明 | 问题、方案、关键规则、验收口径；若涉及状态或权限，仍需补状态/权限差异说明 |

简化不等于省略关键规则。凡是会影响研发实现、测试用例或用户权限的字段、状态、按钮、校验、联动、异常，都必须写清楚。
...
- [ ] 10.1 有状态流转的实体都有状态机
- [ ] 有状态流转的实体都有状态字段可编辑性矩阵，或明确说明不涉及状态差异
